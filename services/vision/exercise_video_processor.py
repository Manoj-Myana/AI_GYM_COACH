...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def set_exercise(self, exercise_type):
        with self._lock:
            if exercise_type != self._exercise_type:
                old_detector = self._detectors.get(self._exercise_type)

                if old_detector:
                    old_detector.reset()

                new_detector = self._detectors.get(exercise_type)
    ...
    def _draw_overlays(self, img, metrics, ex_type):
        if ex_type=="Squats":
            ...
            self._draw_pushups_overlays(img, metrics)
        elif ex_type=="Biceps Curls (Dumbbell)":
            self._draw_biceps_curls_overlays(img, metrics)
        elif ex_type=="Shoulder Press":
            self._draw_shoulder_press_overlays(img, metrics)
        elif ex_type=="Lunges":
    ...
    def _draw_biceps_curls_overlays(self, img, metrics):
        h, _=img.shape[:2]
    ...
    def _draw_shoulder_press_overlays(self, img, metrics):
        h, _=img.shape[:2]
