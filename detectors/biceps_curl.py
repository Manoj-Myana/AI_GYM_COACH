...
class BicepsCurlDetector(BaseExercise):
    ...
    def process(self, landmarks)->dict:
        ...
        if left_vis >= right_vis:
            sholder_idx=self.LEFT_SHOULDER
            elbow_idx=self.LEFT_ELBOW
            wrist_idx=self.LEFT_WRIST
        else:
            sholder_idx=self.RIGHT_SHOULDER
            elbow_idx=self.RIGHT_ELBOW
            wrist_idx=self.RIGHT_WRIST

        elbow_angle=self.calculate_angle(
            self.get_point(landmarks, sholder_idx),
            self.get_point(landmarks, elbow_idx),
            self.get_point(landmarks, wrist_idx)
        )

        key_landmark_visible=landmarks[sholder_idx].visibility >= self.MIN_VISIBILITY and landmarks[elbow_idx].visibility >= self.MIN_VISIBILITY and landmarks[wrist_idx].visibility >= self.MIN_VISIBILITY
        ...
        return {
            ...
            "shoulder_status": sholder_status,
